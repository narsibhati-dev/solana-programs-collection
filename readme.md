...
| --------------------------- | ------------------------------------------------------ |
| `anchor-arcium-hello-world` | Hello World program using Anchor framework with Arcium |
| `anchor-bonding-curve`      | Bonding curve implementation using Anchor              |
| `anchor-counterapp`         | Counter application built with Anchor                  |
| `anchor-cpi`                | Cross-Program Invocation (CPI) examples                |
| `anchor-escrow`             | Escrow program implementation                          |
| `anchor-escrow-blueshift`   | Escrow program with Blueshift integration              |
| `anchor-nft-metaplex`       | NFT program using Metaplex                             |
| `anchor-p-nft`              | Programmable NFT (pNFT) implementation                 |
| `anchor-pda`                | Program Derived Address (PDA) examples                 |
| `anchor-pda-crud`           | PDA with CRUD operations                               |
| `anchor-sol-vault`          | SOL vault program                                      |
| `anchor-tic-tac-toe`        | Tic-tac-toe game on Solana                             |
| `anchor-vault-blueshift`    | Vault program with Blueshift                           |
| `anchor-vault-manager`      | Vault management program                               |
...
- Start with `anchor-hello-world` or `pinocchio-hello-world`
- Try `anchor-counterapp` or `native-counter`
- Explore `anchor-pda` or `native-pda`

### Intermediate

- Study `anchor-cpi` and `native-cpi-basic` for cross-program calls
- Implement escrow with `anchor-escrow` or `native-escrow`
- Learn vault patterns with `anchor-sol-vault`
